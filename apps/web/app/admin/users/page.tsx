...
export default function AdminUsersPage() {
        ...
        try {
            if (currentUser?.role === 'superadmin' || currentUser?.role === 'admin') {
                const [res, conversationsRes] = await Promise.all([
                    api.admin.getUserDetails(user.id),
                    api.admin.getUserConversations(user.id),
                ]);
                setSelectedUserDetails(res);
                const mergedConversations = [
                    ...(conversationsRes?.customerConversations || []).map((conversation: any) => ({
                        ...conversation,
                        adminConversationType: 'customer',
                    })),
                    ...(conversationsRes?.businessConversations || []).map((conversation: any) => ({
                        ...conversation,
                        adminConversationType: 'business',
                    })),
                ];
                setAdminConversations(mergedConversations);
    ...
    const loadConversationNotes = useCallback(async (conversationId: string) => {
        setNotesLoading(true);
        try {
            const res = await chatApi.getNotes(conversationId);
            setConversationNotes(Array.isArray(res) ? res : []);
        } catch (err) {
            console.error('Failed to fetch admin conversation notes', err);
            setConversationNotes([]);
        } finally {
    ...
    useEffect(() => {
        if (selectedConversationId && isDetailsModalOpen) {
            loadConversationNotes(selectedConversationId);
        } else {
            setConversationNotes([]);
        }
    }, [selectedConversationId, isDetailsModalOpen, loadConversationNotes]);

    const handleCreateAdminNote = async () => {
        const cleanNote = noteInput.trim();
        if (!cleanNote || !selectedConversationId) return;

        setNoteSaving(true);
        try {
            await chatApi.createNote(selectedConversationId, cleanNote);
            setNoteInput('');
                                            ...
                                            <div className="rounded-3xl border border-violet-100 bg-violet-50/40 p-5 space-y-5">
                                                ...
                                                {adminConversations.length === 0 ? (
                                                    <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-4 py-5 text-sm font-medium text-slate-400">
                                                        No customer conversations are linked to this account yet, so there are no private notes to review.
                                                    </div>
                                                ) : (
                                                    <>
                                                        <div className="grid gap-3 md:grid-cols-2">
                                                            {adminConversations.map((conversation) => {
                                                                ...
                                                                const counterpartName =
                                                                    conversation.adminConversationType === 'customer'
                                                                        ? conversation.business?.title || 'Business conversation'
                                                                        : conversation.user?.fullName || 'Customer conversation';
                                                                const metaLine =
                                                                    conversation.adminConversationType === 'customer'
                                                                        ? 'Customer-side conversation'
                                                                        : 'Business inbox conversation';
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        <div className="flex items-start justify-between gap-3">
                                                                            <div className="min-w-0">
                                                                                <p className="truncate text-sm font-black text-slate-900">{counterpartName}</p>
                                                                                <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-violet-500">
                                                                                    {metaLine}
                                                                                </p>
                                                                            </div>
                                                                        ...
                                                                        <p className="mt-2 line-clamp-2 text-xs font-medium text-slate-500">
                                                                            {conversation.lastMessage || 'No messages yet in this thread.'}
                                                                        </p>
                                                                    </button>
                                                                );
                                                            })}
                                                        ...
                                                        <div className="rounded-2xl border border-white bg-white p-4 shadow-sm">
                                                            <div className="mb-3 flex items-center justify-between gap-3">
                                                                ...
                                                                {selectedConversationId && (
                                                                    <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                                                        {conversationNotes.length} note{conversationNotes.length === 1 ? '' : 's'}
                                                                    </span>
                                                                )}
                                                            </div>
                                                            ...
                                                            {notesLoading ? (
                                                                ...
                                                                <div className="space-y-3">
                                                                    {conversationNotes.length === 0 ? (
                                                                        <div className="rounded-2xl border border-dashed border-slate-200 px-4 py-4 text-sm font-medium text-slate-400">
                                                                            No private notes have been added to this conversation yet.
                                                                        </div>
                                                                    ) : (
                                                                            ...
                                                                            <div key={note.id} className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
                                                                                <div className="flex items-center justify-between gap-3">
                                                                                    <p className="text-xs font-black text-slate-900">
                                                                                        {note.createdByUser?.fullName || note.createdByUser?.email || 'Admin / Business'}
                                                                                    </p>
                                                                                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                                                                                        {formatDate(note.createdAt)}
                                                                                    </p>
                                                                                </div>
                                                                                <p className="mt-2 text-sm font-medium leading-relaxed text-slate-600">{note.content}</p>
                                                                            </div>
                                                                        ))
                                                                    )}
                                                                </div>
                                                            ...
                                                            <div className="mt-4 space-y-3">
                                                                <textarea
                                                                    ...
                                                                    rows={3}
                                                                    placeholder="Add a private follow-up note for this customer conversation..."
                                                                    className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 outline-none transition focus:border-violet-300 focus:ring-2 focus:ring-violet-100"
                                                                />
                                                                <div className="flex justify-end">
                                                                    <button
                                                                        type="button"
                                                                        onClick={handleCreateAdminNote}
                                                                        disabled={!selectedConversationId || !noteInput.trim() || noteSaving}
                                                                        className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
                                                                    >
                                                                        {noteSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <MessageSquare className="h-4 w-4" />}
                                                                        Save Private Note
                                                                    </button>
                                                                </div>
                                                            </div>
